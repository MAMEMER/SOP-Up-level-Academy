...
- วิธีคิดคะแนนหมวด Stock (เต็ม 20): ไม่ได้นับ (not_counted) ครั้งที่ 1-2 หัก 10/ครั้ง, ครั้งที่ 3+ หัก 5/ครั้ง · นับเช้าช้าเกิน 4 ชม.จากเวลาเริ่มกะ หัก 2 · StoreHub Difference ไม่เป็น 0 (ตั้งแต่ 1 ส.ค. 2026) หัก 2 ต่อรายการ · แก้ได้ใน 24 ชม.ไม่หัก. ข้อมูลมาจาก StoreHub Stock Take (แถว Completed/In Progress).
...
function fallbackAnswer(q: string): string {
  ...
  if (has("คะแนน stock", "stock คะแนน", "หัก stock", "คะแนนสต๊อก", "not_counted", "difference")
    || (has("stock", "สต๊อก") && has("คะแนน", "หัก", "kpi", "20")))
    return "คะแนนหมวด Stock เต็ม 20: ไม่ได้นับ (not_counted) ครั้งที่ 1-2 หัก 10/ครั้ง, ครั้งที่ 3+ หัก 5/ครั้ง · นับเช้าช้าเกิน 4 ชม.จากเวลาเริ่มกะ หัก 2 · StoreHub Difference ไม่เป็น 0 (ตั้งแต่ 1 ส.ค. 2026) หัก 2/รายการ · แก้ได้ใน 24 ชม.ไม่หัก. ข้อมูลจาก StoreHub Stock Take (แถว Completed/In Progress).";
  ...
  if (has("stock", "สต๊อก", "นับ", "เติม")) return "งาน stock อยู่ในเมนู \"เช็คลิสต์\" (นับ+เติม). ข้อมูล stock ภาพรวมดูที่ \"สรุปเจ้าของร้าน\". วิธีคิดคะแนนหมวด stock ถามว่า \"คะแนน stock คิดยังไง\" ได้เลย.";
