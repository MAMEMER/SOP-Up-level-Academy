...
export async function PerformanceScoreView({ searchParams, basePath = "/admin/performance-score", showAdminBackLink = false, isOwner = false }: PerformanceScoreViewProps) {
  ...
  const checklistKpi = await fetchChecklistKpiInput("bangkae", activePeriod.startDate, activePeriod.endDate);
  const rows =
    activePeriod.id === "custom"
      ? getPerformanceScoreRowsForRange(activePeriod, dailyStore, attendance, checklistKpi.submittedDays, checklistKpi.lateSubmissions)
      : getPerformanceScoreRows(activePeriod.id, dailyStore, attendance, checklistKpi.submittedDays, checklistKpi.lateSubmissions);
